...
export default function CustomerProfile() {
  ...
  const fetchActivities = async () => {
    ...
    try {
      const response = await fetch(`https://finfinphone.com/api-lucky/admin/get_customer_activities.php?customer_id=${id}`);
  ...
  const handleDeleteActivity = async () => {
    ...
    try {
      const response = await fetch('https://finfinphone.com/api-lucky/admin/delete_customer_activity.php', {
        method: 'POST',
        headers: {
      ...
      if (result.status === 'success') {
        ...
        throw new Error(result.message || "ไม่สามารถลบกิจกรรมได้");
      }
    } catch (error) {
  ...
  useEffect(() => {
    async function fetchCustomer() {
      ...
      setLoading(true);
      try {
        const response = await fetch(`https://finfinphone.com/api-lucky/admin/get_customer_detail.php?id=${id}`);
        const result = await response.json();
